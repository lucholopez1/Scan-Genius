...
def generar_id(marca, capacidad, fecha):
    ...
    id_concatenado = id_marca + id_capacidad + id_fecha  # Rellena con ceros si tiene menos de 10 dígitos aun no pasa
...
def filtardo():
    ...
    if codigo_encontrado == True:
        messagebox.showinfo("Búsqueda en la base de datos", f"El producto se representa con el codigo {codigo_buscado} y se encuentran en stock {cantidad} unidades.")
...
def eliminar():
    ...
    if codigo_encontrado==True: 
        ...
        boton1 = tk.Button(mini_interfaz, text="Reset", command=reseteo, bg="lightblue")
...
# esta funcion le va a mostrar la tabla de datos al usuario
def abrir_archivo():
    root=tk.Tk()
    archivo = pd.read_csv('base_de_datos.csv', delimiter=';') 
...
# Crear un Label para el título con fondo azul claro
titulo_1 = tk.Label(interfaz, text="Seleccione la marca del dispositivo", bg="lightblue")
titulo_1.grid(row=0, column=0, columnspan=4, pady=10, sticky="ew")
...
titulo_2 = tk.Label(interfaz, text="Seleccione el almacenamiento del dispositivo", bg="lightblue")
titulo_2.grid(row=2, column=0, columnspan=4, pady=10, sticky="ew")
...
titulo_3 = tk.Label(interfaz, text="Seleccione el rango de tiempo en que salió el dispositivo", bg="lightblue")
titulo_3.grid(row=4, column=0, columnspan=4, pady=10, sticky="ew")
...
opcion_mostrar= tk.Button(interfaz, text="Base de datos", command=abrir_archivo, bg="yellow")
opcion_mostrar.grid(row=6, column=0, padx=10, pady=10)

opcion_buscar = tk.Button(interfaz, text="Buscar", command=filtardo, bg="yellow")
opcion_buscar.grid(row=6, column=1, padx=10, pady=10)

opcion_agregar = tk.Button(interfaz, text="Agregar", command=agregar_seleccion, bg="yellow")
opcion_agregar.grid(row=6, column=2, padx=10, pady=10)

opcion_eliminar = tk.Button(interfaz, text="Eliminar", command=eliminar, bg="yellow")
